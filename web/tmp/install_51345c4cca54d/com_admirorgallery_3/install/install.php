...
if (is_a($modules, 'JSimpleXMLElement') && count($modules->children())) {
	...
	foreach ($modules->children() as $module)
	{
		...
		if (!empty ($mname)) {
			...
			$this->parent->abort(JText::_('Module').' '.JText::_('Install').': '.JText::_('Нет модуля в указанном файле'));
			return false;
		}
		...
		*/
		if (file_exists($this->parent->getPath('extension_root'))&&!$this->parent->getOverwrite()) {
			$this->parent->abort(JText::_('Module').' '.JText::_('Install').': '.JText::_('Другой модуль уже использует этот каталог').': "'.$this->parent->getPath('extension_root').'"');
			return false;
		}
		...
		if (!file_exists($this->parent->getPath('extension_root'))) {
			if (!$created = JFolder::create($this->parent->getPath('extension_root'))) {
				$this->parent->abort(JText::_('Модуль').' '.JText::_('Install').': '.JText::_('Не удалось создать каталог').': "'.$this->parent->getPath('extension_root').'"');
				return false;
			}
		}
		...
		*/
		...
		if ($mtitle && $mposition) {
			...
			if (!$db->Query()) {
				// Install failed, roll back changes
				$this->parent->abort(JText::_('Модуль').' '.JText::_('Install').': '.$db->stderr(true));
				return false;
			}
			$id = $db->loadResult();
...
if (is_a($plugins, 'JSimpleXMLElement') && count($plugins->children())) {
	...
	foreach ($plugins->children() as $plugin)
	{
		...
		if (!empty($pname) && !empty($pgroup)) {
			...
			$this->parent->abort(JText::_('Плагин').' '.JText::_('Install').': '.JText::_('Нет плагина в указанном файле'));
			return false;
		}
		...
		if (!file_exists($this->parent->getPath('extension_root'))) {
			if (!$created = JFolder::create($this->parent->getPath('extension_root'))) {
				$this->parent->abort(JText::_('Плагин').' '.JText::_('Install').': '.JText::_('Не удалось создать каталог').': "'.$this->parent->getPath('extension_root').'"');
				return false;
			}
		}
		...
		*/
		...
		if (!$db->Query()) {
			// Install failed, roll back changes
			$this->parent->abort(JText::_('Плагин').' '.JText::_('Install').': '.$db->stderr(true));
			return false;
		}
		$id = $db->loadResult();
		...
		if ($id) {
			...
			{
				// Install failed, roll back changes
				$this->parent->abort(JText::_('Плагин').' '.JText::_('Install').': '.JText::_('Плагин').' "'.$pname.'" '.JText::_('уже существует!'));
				return false;
			}
...
$rows = 0;
?>
<h2>Admiror Gallery установлено удачно</h2>
<table class="adminlist">
	<thead>
		<tr>
			<th class="title" colspan="2"><?php echo JText::_('Расширение'); ?></th>
			<th width="30%"><?php echo JText::_('Статус'); ?></th>
		</tr>
	</thead>
	<tfoot>
		<tr>
	...
	<tbody>
		<tr class="row0">
			<td class="key" colspan="2"><?php echo 'Admiror Gallery '.JText::_('Компонент'); ?></td>
			<td><strong><?php echo JText::_('Установлен'); ?></strong></td>
		</tr>
...
if (count($status->plugins)) : ?>
		<tr>
			<th><?php echo JText::_('Плагин'); ?></th>
			<th><?php echo JText::_('Группа'); ?></th>
			<th></th>
		</tr>
