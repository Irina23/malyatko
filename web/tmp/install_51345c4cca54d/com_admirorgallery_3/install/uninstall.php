...
if (is_a($plugins, 'JSimpleXMLElement') && count($plugins->children())) {
	...
	foreach ($plugins->children() as $plugin)
	{
		...
		if (!empty($pname) && !empty($pgroup)) {
			...
			$this->parent->abort(JText::_('Плагин').' '.JText::_('Uninstall').': '.JText::_('No plugin file specified'));
			return false;
		}
		...
		$db->setQuery($query);
		if (!$db->query()) {
			JError::raiseWarning(100, JText::_('Плагин').' '.JText::_('Uninstall').': '.$db->stderr(true));
			$retval = false;
		}
...
<h2>Admiror Gallery <?php echo JText::_('Удалено'); ?></h2>
<table class="adminlist">
	...
	<tbody>
		<tr class="row0">
			...
			<td><strong><?php echo JText::_('Удалено'); ?></strong></td>
		</tr>
