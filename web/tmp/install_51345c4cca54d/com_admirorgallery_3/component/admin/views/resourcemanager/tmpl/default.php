...
$ag_resourceManager_installed = JFolder::folders(JPATH_SITE.'/plugins/content/AdmirorGallery/'.$AG_resourceType);// N U
...
'.JText::_('Select ZIP package to install').'&nbsp;[ <b>'.JText::_( 'MAX' ).'&nbsp;'.(JComponentHelper::getParams('com_media')->get('upload_maxsize') / 1000000).'M</b> ]:&nbsp;<input type="file" name="AG_fileUpload" size="50" />
<br /><br />
';
...
<td style="padding:0; background-color:transparent"><input type="checkbox" value="" id="checkAll" /></td>
	      ...
	      <th width="200px" class="title" align="left" nowrap="nowrap">'.JText::_( "TITLE").'</th>
...
foreach ($ag_resourceManager_installed as $ag_resourceManager_Key => $ag_resourceManager_Value) {
	if($ag_resourceManager_Key >= $limitstart && $ag_resourceManager_Key < ($limitstart+$limit)){
		...
		if(JFIle::exists(JPATH_SITE.'/plugins/content/AdmirorGallery/'.$AG_resourceType.'/'.$ag_resourceManager_id.'/details.xml')){// N U
			$ag_resourceManager_xml =& JFactory::getXMLParser( 'simple' );
			$ag_resourceManager_xml->loadFile( JPATH_SITE.'/plugins/content/AdmirorGallery/'.$AG_resourceType.'/'.$ag_resourceManager_id.'/details.xml' );// N U
		...
		<span class="editlinktip hasTip" title="'.$ag_resourceManager_name.'::<img border=&quot;1&quot; src=&quot;'.JURI::root().'plugins/content/AdmirorGallery/'.$AG_resourceType.'/'.$ag_resourceManager_id.'/preview.jpg'.'&quot; name=&quot;imagelib&quot; alt=&quot;&quot; width=&quot;206&quot; height=&quot;145&quot; />">
